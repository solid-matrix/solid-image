...
public static class ImageFileDirectoryExtension
{
    ...
    public static uint[]? GetStripOffsets(this ImageFileDirectory ifd)
    {
        ...
        if (!v16.IsEmpty) return v16.ToArray().Select(a => (uint)a).ToArray();
        var v32 = ifd.GetEntryManyUInt32(TiffTags.StripOffsets);
        if (!v32.IsEmpty) return v32.ToArray();
    ...
    public static Predictor? GetPredictor(this ImageFileDirectory ifd)
    {
        return (Predictor?)ifd.GetEntryUInt16(TiffTags.Predictor);
    }
}